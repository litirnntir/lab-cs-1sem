            ...
            int[,] playGround =
{
        ...
        static int countingDigit(string text)
        {
            int k = 0;
            char[] chars = text.ToCharArray();
            foreach (char symb in chars)
            {
        ...
        static void menuString()
        {
            ...
            int k;
            Console.Clear();
            Console.WriteLine("Нажмите цифру для выбора:\n1 - Ввод строки с консоли\n2 - Работа с готовой строкой");
            do
            {
                while (!int.TryParse(Console.ReadLine(), out stringmenu))
                {
                    Console.Write("Неверный формат!\nВведите число: ");
                }
                if (stringmenu == 1)
